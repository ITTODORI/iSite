...
const OptionWheel = ({
  ...
  const runFrame = useCallback((now: number) => {
    ...
    const tiltRad = (cfg.tilt * Math.PI) / 250;
  ...
  return (
    <div
      ...
      className={`relative h-full w-full select-none overflow-hidden outline-none [mask-image:linear-gradient(to_bottom,transparent,black_35%,black_75%,transparent)] [touch-action:none] ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}${className ? ` ${className}` : ''}`}
      style={
        {
