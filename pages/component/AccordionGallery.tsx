...
const DEFAULT_ITEMS: AccordionGalleryItem[] = [
  { image: '/src/Intel.png', label: 'Cyber Monitor', link: '#' },
  { image: '/src/University.png', label: 'University', link: '#' },
  { image: 'https://picsum.photos/id/1039/900/1200', label: 'Dance', link: '#' },
...
const AccordionGallery = ({
  ...
  const [modalItem, setModalItem] = useState<AccordionGalleryItem | null>(null);
  ...
  return (
    <>
      ...
      {modalItem && (
        ...
        >
          <div 
            className="relative max-w-4xl w-full max-h-[90vh] bg-[#121212] border border-white/10 rounded-2xl overflow-hidden shadow-2xl flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              type="button"
              onClick={() => setModalItem(null)}
              className="absolute top-4 right-4 z-10 grid h-10 w-10 place-items-center rounded-full bg-black/60 text-white backdrop-blur-sm transition-colors hover:bg-black/90"
            ...
            <div className="relative w-full h-full bg-black flex items-center justify-center">
              <img
                src={modalItem.image}
                alt={modalItem.alt || modalItem.label || ''}
                className="max-h-full max-w-full object-contain"
              />
            </div>
            {modalItem.label && (
              <div className="p-6 bg-[#18181b]">
                <h3 className="text-xl font-bold text-white tracking-wide">{modalItem.label}</h3>
              </div>
            )}
          </div>
        </div>
